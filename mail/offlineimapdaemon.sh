...
# This script will check a if offlineimap is not running and an internet connections exists. In this
# case it will check the file ~/.offlineimap_lastlongsynctime for the last (unix-) time of a full
# sync.  If this is shorter than ${longsyncdiff} ago then do a quick sync, otherwise do a long one.

# Additionally this will send mail that has been saved using msmtp-queing everytime it finds an
# internet connection.

## WARNING: No guarentees. This might eat your mail in the case that it wrongly detects that
## offlineimap is not running and runs it again. (although offlineimap will also try to catch that case)

# -o run only once
# -f specify folders
# -u choose interface to be quit except in case of erros
